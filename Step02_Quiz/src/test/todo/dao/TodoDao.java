...
/*
 *	application 전역에 대해서 Dao 객체는 오직 한번만 생성되는 구조로 바꾸어야 한다.
 *
...
public class TodoDao {
	...
	public List<TodoDto> getList(TodoDto dto){
		...
		try {
			...
			while (rs.next()) {
				TodoDto tmp2=new TodoDto();
				tmp2.setNum(rs.getInt("num"));
				tmp2.setContent(rs.getString("content"));
				tmp2.setRegdate(rs.getString("regdate"));
		...
		return list; //return list, null 하면 오류남
	}//List END
	...
	public boolean updateDto(MemberDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
