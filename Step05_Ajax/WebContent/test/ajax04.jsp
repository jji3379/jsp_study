...
<body>
	...
	<p class="bbb">form 에 submit 이벤트가 일어 났을 때 폼 전송을 막고 ajax 전송해 보기</p>
	...
	<script>
		// id 가 myForm 인 곳에 submit 이벤트가 일어 났을 때 실행되는 함수 등록
		...
		.addEventListener("submit",function(event){
			...
			//폼에 입력한 이름과 주소 읽어오기
			// "input[name=name]" 은 input요소이면서 name 속성값이 name 인 요소 찾기
			let name=document.querySelector("input[name=name]").value;
			// "input[name=addr]" 은 input요소이면서 name 속성값이 addr 인 요소 찾기			
